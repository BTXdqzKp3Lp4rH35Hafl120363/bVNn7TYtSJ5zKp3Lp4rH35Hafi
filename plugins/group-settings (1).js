...
// ==========================================
// 🛡️ SUPER ROBUST ADMIN CHECKER (Bypasses Device IDs like :5)
// ==========================================
const checkAdmins = async (conn, from, sender) => {
    try {
        ...
        // JID Cleaning: Removing ":5" or any linked device code
        const botNumber = conn.user.id ? conn.user.id.split(':')[0] + '@s.whatsapp.net' : '';
        const senderNumber = sender ? sender.split(':')[0] + '@s.whatsapp.net' : '';

        // Check if Bot is Admin
        const isBotAdmins = participants.some(p => 
            (p.id.split(':')[0] + '@s.whatsapp.net') === botNumber && 
            (p.admin === 'admin' || p.admin === 'superadmin')
        );
        
        // Check if Sender is Admin (or if sender is the bot itself!)
        const isAdmins = participants.some(p => 
            (p.id.split(':')[0] + '@s.whatsapp.net') === senderNumber && 
            (p.admin === 'admin' || p.admin === 'superadmin')
        ) || senderNumber === botNumber; // Bot is always considered its own admin
...
// ==================== TAGALL COMMAND ====================
// Note: Bot does NOT need to be admin to tag members
cmd({
    pattern: "tagall",
    react: "🔊",
    alias: ["gc_tagall"],
...
// ==================== GROUP INFO COMMAND ====================
// Note: Bot does NOT need to be admin to fetch group info
cmd({
    pattern: "ginfo",
    alias: ["groupinfo"],
...
cmd({
    ...
    try {
        ...
        const ownerJid = config.OWNER_NUMBER + '@s.whatsapp.net';
        const ignoreJids = [perms.botNumber, sender.split(':')[0] + '@s.whatsapp.net', ownerJid];
        
        const targets = perms.participants.filter(p => !ignoreJids.includes(p.id.split(':')[0] + '@s.whatsapp.net'));
        const jids = targets.map(p => p.id);
