...
cmd({
    ...
    react: "🔊",
    filename: __filename
}, async (conn, mek, m, { from, isCreator, isGroup, reply, isBotAdmins, isAdmins }) => {
    try {
...
cmd({
    ...
    use: '.tagall [message]',
    filename: __filename
}, async (conn, mek, m, { from, isGroup, isCreator, reply, isAdmins, participants, groupName, body, command }) => {
    try {
        ...
        let teks = `▢ Group : *${groupName || "Unknown"}*\n▢ Members : *${totalMembers}*\n▢ Message: *${message}*\n\n┌───⊷ *MENTIONS*\n`;
...
cmd({
    ...
    react: "💀",
    filename: __filename
}, async (conn, mek, m, { from, isCreator, isGroup, reply, isBotAdmins, isAdmins, botNumber2 }) => {
    try {
        if (!isGroup) return await reply("⚠️ This command only works in groups.");
        if (!isBotAdmins) return await reply("❌ I must be admin to remove someone.");
        if (!isAdmins && !isCreator) return await reply("🔐 Only bot owner or group admins can use this command.");
        ...
        if (targetUser === botNumber2) return await reply("🤖 I can't kick myself!");
...
cmd({
    ...
    try {
        ...
        if (targetUser === botNumber2) return await reply("🤖 I am already admin!");
        const ownerJid = config.OWNER_NUMBER + '@s.whatsapp.net';
...
cmd({
    ...
    try {
        ...
        if (targetUser === botNumber2) return await reply("🤖 I can't demote myself!");
        const ownerJid = config.OWNER_NUMBER + '@s.whatsapp.net';
...
cmd({
    ...
    react: "🔗",
    filename: __filename
}, async (conn, mek, m, { from, isGroup, reply, isBotAdmins }) => {
    try {
        if (!isGroup) return await reply("⚠️ This command only works in groups.");
...
cmd({
    ...
    react: "🥏",
    filename: __filename
}, async (conn, mek, m, { from, isGroup, reply, groupName, participants, groupAdmins }) => {
    try {
        if (!isGroup) return await reply("⚠️ This command only works in groups.");
        ...
        groupAdmins.forEach((admin, i) => {
            text += `${i+1}. @${admin.split('@')[0].split(':')[0]}\n`;
        });
        ...
        try {
            ...
            await reply(text, { mentions: groupAdmins });
        }
    } catch (err) {
...
cmd({
    ...
    react: "📜",
    filename: __filename
}, async (conn, mek, m, { from, isCreator, isGroup, q, reply, isBotAdmins, isAdmins }) => {
    try {
        if (!isGroup) return await reply("⚠️ This command only works in groups.");
...
cmd({
    ...
    category: "group",
    filename: __filename,
}, async (conn, mek, m, { from, isCreator, isGroup, q, reply, isAdmins }) => {
    try {
        if (!isGroup) return await reply("⚠️ This command only works in groups.");
...
cmd({
    ...
    react: "❌",
    filename: __filename
}, async (conn, mek, m, { from, q, isGroup, isCreator, reply, isBotAdmins, participants }) => {
    if (!isGroup) return reply("❌ This command can only be used in groups.");
...
cmd({
    ...
    category: "group",
    filename: __filename,
}, async (conn, mek, m, { isCreator, body, reply, botNumber2 }) => {
    try {
        ...
        const participantNumbers = numbersString.split(",").map(num => {
            let cleanNum = num.trim().replace(/[^0-9]/g, '');
        ...
        if (!participantNumbers.includes(botNumber2)) participantNumbers.push(botNumber2);
...
cmd({
    ...
    react: "⚠️",
    filename: __filename
}, async (conn, mek, m, { from, isCreator, isGroup, reply, isBotAdmins, participants, botNumber2, senderNumber }) => {
    try {
        if (!isGroup) return await reply("⚠️ This command only works in groups.");
        if (!isCreator) return await reply("🔐 Only bot owner can use this command.");
        ...
        const ownerJid = config.OWNER_NUMBER + '@s.whatsapp.net';
        const ignoreJids = [botNumber2, senderNumber + '@s.whatsapp.net', ownerJid];
        
        const targets = participants.filter(p => !ignoreJids.includes(p.id.split('@')[0].split(':')[0] + '@s.whatsapp.net'));
        const jids = targets.map(p => p.id);
...
cmd({
    ...
    filename: __filename,
    react: "📨"
}, async (conn, mek, m, { from, args, isGroup, isCreator, reply, isBotAdmins, isAdmins, groupName }) => {
    try {
        ...
        await conn.sendMessage(jid, { text: `📨 *You're invited to join ${groupName}*\n\n🔗 ${link}\n\n👤 Invited by: @${m.sender.split('@')[0].split(':')[0]}` });
...
cmd({
    ...
    filename: __filename
}, async (conn, mek, m, { from, q, isGroup, isCreator, reply, participants }) => {
    try {
        if (!isGroup) return reply("❌ This command can only be used in groups.");
...
cmd({
    ...
    filename: __filename
}, async (conn, mek, m, { from, q, isGroup, isCreator, reply, isAdmins, participants }) => {
    try {
        if (!isGroup) return reply("❌ This command can only be used in groups.");
...
cmd({
    ...
    react: "✅",
    filename: __filename
}, async (conn, mek, m, { from, isCreator, isGroup, args, reply, isBotAdmins, isAdmins }) => {
    try {
        if (!isGroup) return await reply("⚠️ This command only works in groups.");
        if (!isBotAdmins) return await reply("❌ I must be admin to accept join requests.");
        if (!isAdmins && !isCreator) return await reply("🔐 Only admins can use this command.");
...
cmd({
    ...
    filename: __filename,
    react: "➕"
}, async (conn, mek, m, { from, args, quoted, mentionedJid, isGroup, isCreator, reply, isBotAdmins }) => {
    try {
        if (!isGroup) return await reply("⚠️ Group only.");
        if (!isBotAdmins) return await reply("❌ I need admin.");
