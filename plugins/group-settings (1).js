...
// ==========================================
// 🛡️ SUPER ROBUST ADMIN & GROUP PERMISSION CHECKER (Fixed Device ID Bug)
// ==========================================
async function getGroupPerms(conn, from, sender) {
    try {
        ...
        // 1. Normalize Bot and Sender JID (Remove device IDs like :5)
        const botNum = conn.user.id ? conn.user.id.split(':')[0] : '';
        const botJid = botNum + '@s.whatsapp.net';
        const senderJid = sender ? sender.split(':')[0] + '@s.whatsapp.net' : '';
        
        // 2. Normalize Participant IDs and extract true Admins
        const groupAdmins = participants
            .filter(p => p.admin === 'admin' || p.admin === 'superadmin')
            .map(p => p.id.split(':')[0] + '@s.whatsapp.net'); 
        
        // 3. Perfect matching
        const isBotAdmins = groupAdmins.includes(botJid) || conn.user.id === sender; // Bot itself is super bypass
...
cmd({
    ...
    try {
        ...
        // Normalize target for safety
        const safeTarget = targetUser.split(':')[0] + '@s.whatsapp.net';
        
        if (safeTarget === botJid) return await reply("🤖 I can't kick myself!");
        
        const ownerJid = config.OWNER_NUMBER + '@s.whatsapp.net';
        if (safeTarget === ownerJid) return await reply("👑 That's the owner! I can't remove them.");

        await conn.groupParticipantsUpdate(from, [targetUser], "remove");
        await reply(`*✅ Successfully removed from group.*`, { mentions: [targetUser] });
    } catch (err) {
        console.error(err);
        await reply("❌ Failed to remove user. Privacy settings might be blocking it.");
    }
});
...
cmd({
    ...
    try {
        ...
        const safeTarget = targetUser.split(':')[0] + '@s.whatsapp.net';
        if (safeTarget === botJid) return await reply("🤖 I can't promote myself (I am already admin)!");
...
cmd({
    ...
    try {
        ...
        const safeTarget = targetUser.split(':')[0] + '@s.whatsapp.net';
        if (safeTarget === botJid) return await reply("🤖 I can't demote myself!");
        
        const ownerJid = config.OWNER_NUMBER + '@s.whatsapp.net';
        if (safeTarget === ownerJid) return await reply("👑 I can't demote the owner!");
...
cmd({
    ...
    desc: "Removes all members from group",
    category: "group",
    react: "⚠️",
    filename: __filename
}, async (conn, mek, m, { from, isCreator, isGroup, sender, reply }) => {
    try {
        ...
        const ownerJid = config.OWNER_NUMBER + '@s.whatsapp.net';
        const ignoreJids = [botJid, sender.split(':')[0] + '@s.whatsapp.net', ownerJid];
        
        const targets = participants.filter(p => !ignoreJids.includes(p.id.split(':')[0] + '@s.whatsapp.net'));
        const jids = targets.map(p => p.id);
