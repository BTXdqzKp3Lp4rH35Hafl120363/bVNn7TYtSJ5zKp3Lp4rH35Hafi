...
// ==========================================
// 🛡️ SUPER ROBUST ADMIN CHECKER (Fixed JID Formatting & Bypass)
// ==========================================
const checkAdmins = async (conn, from, sender) => {
    try {
        const groupMetadata = await conn.groupMetadata(from);
        const participants = groupMetadata.participants || [];
        
        // 🔥 FIXED: Strip linked device codes but DO NOT duplicate @s.whatsapp.net
        const botNumber = conn.user.id ? conn.user.id.split('@')[0].split(':')[0] + '@s.whatsapp.net' : '';
        const senderNumber = sender ? sender.split('@')[0].split(':')[0] + '@s.whatsapp.net' : '';

        // Check if Bot is Admin
        const isBotAdmins = participants.some(p => 
            (p.id.split('@')[0].split(':')[0] + '@s.whatsapp.net') === botNumber && 
            (p.admin === 'admin' || p.admin === 'superadmin')
        );
        
        // Check if Sender is Admin (OR bot owner / bot itself bypass)
        const isAdmins = participants.some(p => 
            (p.id.split('@')[0].split(':')[0] + '@s.whatsapp.net') === senderNumber && 
            (p.admin === 'admin' || p.admin === 'superadmin')
        ) || senderNumber === botNumber; // "Jis number pe bot chal raha ho wo always admin hoga"

        return { isBotAdmins, isAdmins, participants, groupMetadata, botNumber, senderNumber };
    } catch (error) {
        return { isBotAdmins: false, isAdmins: false, participants: [], botNumber: '' };
    }
};
...
cmd({
    ...
    try {
        ...
        const perms = await checkAdmins(conn, from, sender);

        if (!perms.isBotAdmins) return await reply("❌ I must be admin to unmute the group.");
        if (!perms.isAdmins && !isCreator) return await reply("🔐 Only group admins or owner can use this command.");
...
cmd({
    ...
    try {
        ...
        if (!perms.isBotAdmins) return await reply("❌ I must be admin to mute the group.");
        if (!perms.isAdmins && !isCreator) return await reply("🔐 Only group admins or owner can use this command.");
...
cmd({
    ...
    try {
        ...
        if (!perms.isAdmins && !isCreator) return reply("❌ Only group admins or the bot owner can use this command.");
        if (!perms.groupMetadata) return reply("❌ Failed to fetch group information.");

        let groupName = perms.groupMetadata.subject || "Unknown Group";
        let totalMembers = perms.participants.length;
        ...
        for (let mem of perms.participants) {
            if (!mem.id) continue;
            teks += `${randomEmoji} @${mem.id.split('@')[0].split(':')[0]}\n`;
        }

        teks += `└──✪ ${config.BOT_NAME} ✪──`;

        conn.sendMessage(from, { text: teks, mentions: perms.participants.map(a => a.id) }, { quoted: mek });
    } catch (e) {
...
cmd({
    ...
    try {
        ...
        const perms = await checkAdmins(conn, from, sender);
        const mentionedJid = perms.participants.map(p => p.id);
...
cmd({
    ...
    try {
        ...
        if (!perms.isBotAdmins) return await reply("❌ I must be admin to remove someone.");
        if (!perms.isAdmins && !isCreator) return await reply("🔐 Only bot owner or group admins can use this command.");
        ...
        // 🔥 FIXED JID Extraction completely
        targetUser = targetUser.split('@')[0].split(':')[0] + '@s.whatsapp.net';

        if (targetUser === perms.botNumber) return await reply("🤖 I can't kick myself!");
...
cmd({
    ...
    try {
        ...
        if (!perms.isBotAdmins) return await reply("❌ I must be admin to promote someone.");
        if (!perms.isAdmins && !isCreator) return await reply("🔐 Only group admins or owner can use this command.");
        ...
        if (targetUser === perms.botNumber) return await reply("🤖 I am already admin!");
        const ownerJid = config.OWNER_NUMBER + '@s.whatsapp.net';
        if (targetUser === ownerJid) return await reply("👑 Owner is already super admin!");
...
cmd({
    ...
    try {
        ...
        if (!perms.isBotAdmins) return await reply("❌ I must be admin to demote someone.");
        if (!perms.isAdmins && !isCreator) return await reply("🔐 Only group admins or owner can use this command.");
        ...
        if (targetUser === perms.botNumber) return await reply("🤖 I can't demote myself!");
        const ownerJid = config.OWNER_NUMBER + '@s.whatsapp.net';
...
cmd({
    ...
    try {
        ...
        if (!perms.isBotAdmins) return await reply("❌ I must be admin to change group picture.");
        if (!perms.isAdmins && !isCreator) return await reply("🔐 Only admins can use this command.");
...
cmd({
    ...
    try {
        ...
        if (!perms.isBotAdmins) return await reply("❌ I must be admin to reset link.");
        if (!perms.isAdmins && !isCreator) return await reply("🔐 Only admins can use this command.");
...
cmd({
    ...
    try {
        ...
        if (!perms.isBotAdmins) return await reply("❌ I must be admin to get the invite link.");
...
cmd({
    ...
    try {
        ...
        if (!perms.groupMetadata) return await reply("❌ Failed to fetch group information.");

        const groupAdmins = perms.participants.filter(p => p.admin === 'admin' || p.admin === 'superadmin') || [];
        let description = perms.groupMetadata.desc || 'No description';
        
        let text = `*「 Group Information 」*\n\n`;
        text += `*Name:* ${perms.groupMetadata.subject}\n`;
        text += `*ID:* ${perms.groupMetadata.id}\n`;
        text += `*Participants:* ${perms.participants.length}\n`;
        text += `*Created:* ${new Date(perms.groupMetadata.creation * 1000).toLocaleString()}\n\n`;
        text += `*Description:*\n${description}\n\n`;
        text += `*Admins (${groupAdmins.length}):*\n`;
        
        groupAdmins.forEach((admin, i) => {
            text += `${i+1}. @${admin.id.split('@')[0].split(':')[0]}\n`;
        });
...
cmd({
    ...
    react: "📜",
    filename: __filename
}, async (conn, mek, m, { from, isCreator, isGroup, q, sender, reply }) => {
    try {
        if (!isGroup) return await reply("⚠️ This command only works in groups.");
        const perms = await checkAdmins(conn, from, sender);

        if (!perms.isBotAdmins) return await reply("❌ I must be admin to change group description.");
        if (!perms.isAdmins && !isCreator) return await reply("🔐 Only admins can use this command.");
...
cmd({
    ...
    try {
        ...
        if (!perms.isBotAdmins) return await reply("❌ I must be admin to change group name.");
        if (!perms.isAdmins && !isCreator) return await reply("🔐 Only admins can use this command.");
...
cmd({
    ...
    try {
        ...
        if (!perms.isAdmins && !isCreator) return await reply("🔐 Only admins can create polls.");
...
cmd({
    ...
    if (!perms.isBotAdmins) return reply("❌ I need to be an admin to use this command.");
    ...
    try {
        const targets = perms.participants.filter(participant => participant.id.startsWith(countryCode) && !participant.admin);
...
cmd({
    ...
    try {
        ...
        const participantNumbers = numbersString.split(",").map(num => {
            let cleanNum = num.trim().replace(/[^0-9]/g, ''); // Extract valid numbers only
            if (cleanNum.startsWith("3")) cleanNum = "92" + cleanNum;
            return `${cleanNum}@s.whatsapp.net`;
        }).filter(num => num.length >= 15);
...
cmd({
    ...
    try {
        ...
        const perms = await checkAdmins(conn, from, sender);
        if (!perms.isBotAdmins) return await reply("❌ I must be admin to remove members.");

        const ownerJid = config.OWNER_NUMBER + '@s.whatsapp.net';
        const ignoreJids = [perms.botNumber, sender.split('@')[0].split(':')[0] + '@s.whatsapp.net', ownerJid];
        
        const targets = perms.participants.filter(p => !ignoreJids.includes(p.id.split('@')[0].split(':')[0] + '@s.whatsapp.net'));
        const jids = targets.map(p => p.id);
...
cmd({
    ...
    try {
        ...
        if (!perms.isBotAdmins) return await reply("❌ I need admin.");
        if (!isCreator && !perms.isAdmins) return await reply("🔐 Admins only.");
        ...
        await conn.sendMessage(jid, { text: `📨 *You're invited to join ${perms.groupMetadata.subject}*\n\n🔗 ${link}\n\n👤 Invited by: @${m.sender.split('@')[0].split(':')[0]}` });
...
cmd({
    ...
    alias: ["h"],
    react: "🔇",
    desc: "Hidden tag with custom message (Creator only)",
    category: "owner",
...
cmd({
    ...
    alias: ["taggc"],
    react: "🔊",
    desc: "Tag all members (Admins & Creator)",
    category: "group",
    ...
    try {
        ...
        if (!perms.isAdmins && !isCreator) return reply("❌ Only group admins can use this command.");
...
cmd({
    ...
    try {
        ...
        if (!perms.isBotAdmins) return await reply("❌ I must be admin to accept join requests.");
        if (!perms.isAdmins && !isCreator) return await reply("🔐 Only admins can use this command.");
...
cmd({
    ...
    try {
        ...
        if (!perms.isBotAdmins) return await reply("❌ I must be admin to reject join requests.");
        if (!perms.isAdmins && !isCreator) return await reply("🔐 Only admins can use this command.");
...
cmd({
    ...
    try {
        ...
        if (!perms.isBotAdmins) return await reply("❌ I must be admin to view join requests.");
        if (!perms.isAdmins && !isCreator) return await reply("🔐 Only admins can use this command.");
...
cmd({
    ...
    react: "✅",
    filename: __filename
}, async (conn, mek, m, { from, isCreator, isGroup, args, sender, reply }) => {
    try {
        if (!isGroup) return await reply("⚠️ This command only works in groups.");
        const perms = await checkAdmins(conn, from, sender);
