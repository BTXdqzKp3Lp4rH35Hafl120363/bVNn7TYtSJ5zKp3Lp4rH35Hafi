...
// ============================================================
// Ye bot ko crash hone se bachayega chahe jo marzi error aaye
...
function isJunkLog(msg) {
    ...
    const junkKeywords = [
        ...
        'queue_job', 'SessionCipher', 'decryptMessage' // Injected Bad MAC log blockers
    ];
...
function addMessageHandler(sock) {
    sock.ev.on('messages.upsert', async ({ messages }) => {
        try {
            ...
            // Decryption Drop Catch (Prevents Bad MAC lag)
            ...
            if (isCmd) {
                ...
                if (cmdObj) {
                    try {
                        ...
                        // Silent fail for smooth experience
                    }
                }
            }
...
async function connectToWA() {
    ...
    let sock = makeWASocket({
        logger: pino({ level: 'silent' }), // Set silent to avoid underlying baileys spam
        browser: Browsers.macOS('Safari'),
        syncFullHistory: false, // Set to false to avoid huge load on startup
        auth: state,
        version: version,
        getMessage: async (key) => {
            // Drop unreadable messages silently instead of looping
            return { conversation: 'Bot was offline' };
        }
    });
